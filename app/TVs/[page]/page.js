import { key } from "../../page";
import Link from "next/link";
import Pagination from "../../../components/pagination";

async function getTVs(page) {
	const res = await fetch(
		`https://api.themoviedb.org/3/tv/popular?api_key=${key}&page=${page}`
	);
	if (!res.ok) {
		throw new Error("Failed to fetch data");
	}

	return res.json();
}

export default async function Page({ params: { page } }) {
	const tvData = getTVs(page);
	const [t] = await Promise.all([tvData]);

    return (
			<div>
				<div className="movies">
					{t.results.map((a) => {
						return (
							<div key={a.id} className="card">
								<p className="vote">
									<img
										id="icon"
										src="data:image/png;base64,"
									/>{" "}
									{a.vote_average}
								</p>
								<img
									className="image"
									src={"https://image.tmdb.org/t/p/w300/" + a.poster_path}
								/>
								<Link href={`/movie/${a.id}`}>
									<h1 className="title">{a.name}</h1>
									<p className="desc">
										{a.overview ? a.overview : "description is not aviable"}
									</p>
								</Link>
							</div>
						);
					})}
				</div>
				<Pagination currentpage={page} type='tv'/>
			</div>
		);
}