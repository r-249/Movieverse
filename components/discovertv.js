import { key } from "../app/page";
import Link from "next/link";

async function getDiscover() {
	const res = await fetch(
		`https://api.themoviedb.org/3/discover/tv?api_key=${key}&page=1`
	);
	if (!res.ok) {
		throw new Error("Failed to fetch data");
	}

	return res.json();
}

export default async function DiscoverTV() {
     const list = await getDiscover();
	return (
		<div>
			<h1 className="mini-title">Discover TVs</h1>
			<div className="movies">
				{list.results.map((a) => {
					return (
						<div key={a.id} className="card">
							<p className="vote">
								<img
									id="icon"
									src="data:image/png;base64,"
								/>{" "}
								{a.vote_average}
							</p>
							<img
								className="image"
								src={"https://image.tmdb.org/t/p/w300/" + a.poster_path}
							/>
							<Link href={`/movie/${a.id}`}>
								<h1 className="title">{a.name}</h1>
								<p className="desc">{a.overview?a.overview:'description is not aviable'}</p>
							</Link>
						</div>
					);
				})}
			</div>
		</div>
	);
}
